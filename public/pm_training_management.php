    ...
    try {
        ...
        $query = "
            SELECT 
                ...
                ptr.status,
                ptr.ptr_file,
                (SELECT COUNT(*) FROM pm_training_attendance WHERE pm_training_request_id = ptr.id AND attended = 1) as attended_count,
                (SELECT COUNT(*) FROM pm_training_attendance WHERE pm_training_request_id = ptr.id) as total_attendees
...
// Fetch all users for attendance - EXCLUDE admins, superadmins, and proponents
$stmt = $pdo->prepare("SELECT id, username, CONCAT(fname, ' ', lname) as fullname 
    FROM users 
...
// Get divisions and departments for filter dropdowns
...
if (!empty($filter_committee)) {
    $where_clause[] = "ud.committee_id = ?";
    $params[] = $filter_committee;
}

if (!empty($search)) {
    $where_clause[] = "(ptr.title LIKE ? OR ptr.venue LIKE ? OR ptr.hospital_order_no LIKE ? OR ptr.remarks LIKE ?)";
    $search_param = "%$search%";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
        ...
        <div class="card mb-4">
            <div class="card-header">
                <h5>
                    ...
                    if (!empty($filter_division)) $active_filters++;
                    if (!empty($filter_dept)) $active_filters++;
            ...
            <div class="card-body">
                <form method="GET" action="" id="filterForm" class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="filter_dept" class="form-select">
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="search" class="form-control" placeholder="Title, venue, order no..." 
        ...
        <?php if ($active_filters > 0): ?>
            <div class="alert alert-info alert-dismissible fade show mb-4" role="alert">
                ...
                <?php 
                    ...
                    if (!empty($filter_division)) {
                        $div = array_filter($divisions, fn($d) => $d['id'] == $filter_division);
                        if (!empty($div)) {
                            $div = reset($div);
                            $filter_texts[] = "Division: " . htmlspecialchars($div['name']);
                        }
                    }
                    if (!empty($filter_dept)) {
                        $dept = array_filter($all_departments, fn($d) => $d['id'] == $filter_dept);
                        if (!empty($dept)) {
                            $dept = reset($dept);
                            $filter_texts[] = "Department: " . htmlspecialchars($dept['name']);
                        }
                    }
        ...
        <div class="table-card">
            <div class="table-card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h4><i class="fas fa-list"></i> PM Training Requests List <span class="badge bg-secondary"><?= count($pm_requests) ?> record<?= count($pm_requests) !== 1 ? 's' : '' ?></span></h4>
                    ...
                    <tbody>
                        <?php if (!empty($pm_requests)): ?>
                            <?php foreach ($pm_requests as $request): ?>
                                ...
                                // Check if attachments are reviewed/complete (status is 'complete' or approved)
                                $attachments_reviewed = ($request['status'] === 'complete') || ($request['status'] === 'approved' && $has_ptr);
                                
                                // Determine row background color based on attachment status and days elapsed
                                // Warning only shows if no PTR file AND attachments are not yet reviewed
                                $row_bg_style = '';
                                if (!$has_ptr && !$attachments_reviewed) {
                                    if ($days_elapsed >= 1 && $days_elapsed <= 28) {
                                        $row_bg_style = 'style="background-color: #fff3cd; border-left: 4px solid #ffc107;"';
                                    } elseif ($days_elapsed >= 29) {
                                        $row_bg_style = 'style="background-color: #ffe6e6; border-left: 4px solid #dc3545;"';
                                    }
                                }
                                ?>
                                <tr>
                            ...
                            <div class="attendee-list" id="addAttendeeList">
                                <?php foreach ($all_users as $u): ?>
                                    <div class="attendee-item">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input attendee-checkbox" name="attendees" value="<?= $u['id'] ?>">
                ...
                <form id="editPmTrainingForm">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12" id="approvalSection">
                            <div class="" id="approveCheckboxSection">
                                <div class="form-check">
                            ...
                            <div class="" id="completeButtonSection" style="display: none;">
...
<!-- Attendance Check Modal - Read-only for Admin -->
            ...
            <div class="modal-body">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <button id="exportPmReportBtn" class="btn btn-info w-100">
...
<script>
    ...
    function setupAttendeeSearch(searchInputId, listContainerId) {
        const searchInput = document.getElementById(searchInputId);
        if (!searchInput) return;

        searchInput.addEventListener('keyup', function() {
            const searchTerm = this.value.toLowerCase();
            const items = document.querySelectorAll(`#${listContainerId} .attendee-item`);
    ...
    setupAttendeeSearch('addAttendeeSearch', 'addAttendeeList');
    setupAttendeeSearch('attendanceListSearch', 'attendanceCheckList');
...
if (approvalSection) {
    ...
    // Open Attendance Modal - Read-only for Admin
        ...
        fetch(url.toString())
            ...
            .then(data => {
                if (data.success) {
                    ...
                    <?php if (is_admin() || is_superadmin()): ?>
                    // Admin view - read-only, shows checkmarks for attended users
                    ...
                    <?php else: ?>
                    // Regular user view - editable checkboxes
                    data.attendees.forEach(attendee => {
                        const div = document.createElement('div');
                        div.className = 'attendee-item';
                        div.innerHTML = `
                            <div class="form-check">
                                ...
                                <label class="form-check-label">
                                    ${escapeHtml(attendee.fullname || attendee.username)}
                                </label>
                            </div>
                        `;
                        attendeeList.appendChild(div);
    ...
    // Load report data - only completed trainings
function loadPmReportData() {
                ...
                data.reports.forEach(report => {
                    ...
                    tbody.innerHTML += `
                        <tr>
                            ...
                            <td>${report.attended_count} / ${report.total_attendees}</div>
                            <td>${ptrLink}</div>
                        </tr>
                    `;
                });
   ...
   // Export report as CSV - only completed trainings
function exportPmReportToCSV() {
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success && data.reports.length > 0) {
                // Create CSV content - removed Status column since all are complete
